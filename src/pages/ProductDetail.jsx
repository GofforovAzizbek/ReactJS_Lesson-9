...
function ProductDetail() {
  ...
  const reviews = useMemo(() => {
    ...
    return all.filter((review) => String(review.productId) === String(product?.id));
  }, [product?.id]);
  ...
  if (error) return <ErrorState className="container mt-8" message={error} onRetry={fetchProduct} />;
  ...
  return (
    <div className="container py-8 sm:py-10">
      <div className="mb-6 text-sm text-black/60">
        ...
        &gt;{" "}
        <Link to={`/shop/${String(style).toLowerCase()}`} className="transition hover:text-black">
          {style}
        </Link>{" "}
      ...
      <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="order-2 flex gap-3 sm:order-1 sm:flex-col">
            {gallery.map((image, index) => (
              <button
                ...
                className={`overflow-hidden rounded-[20px] bg-[#f0f0f0] transition ${
              ...
              >
                <img
                  ...
                  className="h-28 w-28 object-contain p-3 sm:h-[152px] sm:w-[152px]"
                />
              </button>
            ))}
          </div>

          <div className="order-1 flex min-h-[380px] items-center justify-center rounded-[20px] bg-[#f2f0f1] p-8 sm:order-2 sm:min-h-[530px]">
            {currentImage ? (
              <img
                src={currentImage}
        ...
        <div>
          <h1 className="mb-3 text-[32px] font-black uppercase leading-none text-black sm:text-[40px]">
          ...
          <p className="mb-6 border-b border-black/10 pb-6 text-sm leading-6 text-black/60 sm:text-base">
            {product.description || "A clean essential designed for everyday wear and styled to match the look and feel of the storefront."}
          </p>
              ...
              <div className="flex flex-wrap gap-4">
                {colors.map((color, index) => (
                  <button
                    ...
                    className={`flex h-10 w-10 items-center justify-center rounded-full border border-black/10 transition ${
          ...
          <div className="grid gap-3 rounded-[20px] bg-[#f8f8f8] p-5 text-sm text-black/60 sm:grid-cols-2">
            <p>
              <span className="font-semibold text-black">Category:</span> {type}
            </p>
            <p>
              <span className="font-semibold text-black">Dress Style:</span> {style}
            </p>
            <p>
              <span className="font-semibold text-black">Stock:</span> {product.stock || 0}
            </p>
            <p>
              <span className="font-semibold text-black">Brand:</span> {product.brand || "Shop.co"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 sm:mt-16">
        ...
        {selectedTab === "details" ? (
          <div className="grid gap-4 pt-8 text-sm text-black/70 sm:grid-cols-2 sm:text-base">
            <p>
              <span className="font-semibold text-black">Name:</span> {product.name || product.title || "-"}
            </p>
            <p>
            ...
            <p>
              <span className="font-semibold text-black">Selected Size:</span> {selectedSize || "-"}
            </p>
            <p>
              <span className="font-semibold text-black">Discount:</span> {discount}%
            </p>
            <p>
              <span className="font-semibold text-black">Rank:</span> {product.rank ?? 0}
            </p>
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-black/60">
                  Reviews are sourced from the project&apos;s local admin dashboard.
                </p>
              </div>
        ...
        {selectedTab === "faqs" ? (
          <div className="grid gap-4 pt-8">
            {[
              ...
              <div key={item} className="rounded-[20px] border border-black/10 p-5 text-sm text-black/70">
                {item}
              </div>
            ))}
          </div>
